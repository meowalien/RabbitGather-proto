...
OUTPUT_DIR="./proto"    # This is where you want your generated files to reside.
...
find $PROTO_DIR -name "*.proto" | while read -r proto_file; do
  ...
  protoc -I="$PROTO_DIR" --go_out="$OUTPUT_DIR" --go_opt=paths=source_relative --go-grpc_out="$OUTPUT_DIR" --go-grpc_opt=paths=source_relative "$proto_file"
done
